...
class Users extends CI_Controller {
	...
	public function index()
	{
		if ($this->session->userdata('user_logged_in'))
		{
			...
			$this->data['table_data'] = generateTableUsers($tmpdata);
    ...
    public function edit($id)
	{
		if ($this->session->userdata('user_logged_in'))
		{
            ...
            $this->load->model('users_model', 'users');  
            $tmpdata = $this->users->get_user($id);
            $this->data['table_data'] = generateFormUser($tmpdata);
            */
            ...
            if(is_null($data['values'])) {
                ...
                $data['values'] = $this->users->_get_user($this->users->get_user_id_by_uid($id));
            }
    ...
    public function update()
	{
		if ($this->session->userdata('user_logged_in'))
		{
            file_put_contents(FCPATH . "debug.log", "--- UPDATE file ".__FILE__." | function ".__FUNCTION__." ln ".__LINE__." ---\n post data\n".print_r($_POST,1), FILE_APPEND);
            $this->load->model('users_model', 'users');  
            $tmpdata = $this->users->update_user($_POST);
