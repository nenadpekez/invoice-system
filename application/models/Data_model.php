...
class Data_model extends CI_Model {
    ...
    function get_form_data($form=false, $id=false)
    {
        ...
        foreach ($ret['fields'] as $vkey => $vobj) {
            ...
            if($vobj->params) {
                ...
                $sql = sprintf("SELECT * FROM ".$this->db->dbprefix."%s ORDER BY %s"
                        ,$params->table
                        ,$params->field_id
                    );
                $res = $this->db->query($sql);
    ...
    function checkTableName($table_name)
    {
        ...
        $query = $this->db->query("SELECT table_name FROM information_schema.tables WHERE table_schema = '$databasename' AND table_name = '$this->db->dbprefix.$table_name';");
